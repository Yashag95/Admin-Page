...
const OpenTicket = () => {
  ...
  const [filterUsersOpen, setFilterUsersOpen] = useState([]);
  ...
  const getUsersOpen = async () => {
    try {
      const response = await axios.get("http://localhost:5000/open_ticket_table")
      setUsersOpen(response.data)
      setFilterUsersOpen(response.data)
    }
    catch (error) {
      console.log(error);
    }
...
const columns = [
  ...
  <div className="page-title-box" >
    ...
    <h4 className="page-title"><b style={{marginLeft: "20px"}}>Open Ticket</b></h4>
  </div>
      ...
      <DataTable title="Ticket Details" columns={columns} data={filterUsersOpen} pagination fixedHeader
